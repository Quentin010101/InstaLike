<x-layout>
    ...
    <div class="flex relative min-h-screen">
        ...
        <main class="md:ml-64 lg:ml-80 w-full pt-16 bg-slate-100 dark:bg-slate-700">
            @if (session('status'))
                <div>
                    {{ session('status') }}
                </div>
            ...
            @if ($isActiveFeed)
                <div
                    class="relative bg-slate-100 dark:bg-slate-700 px-2 sm:px-4 md:px-6 lg:px-12 xl:px-32 flex flex-col items-center gap-y-5 h-full">
                    <livewire:card-container>
                </div>
            @endif
            @if ($isActiveProfile)
                <div class="mt-8 flex bg-slate-100 dark:bg-slate-700">
