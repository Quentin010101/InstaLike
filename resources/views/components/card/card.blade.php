<div class="bg-white dark:bg-gray-600 pt-12 px-3 md:px-6 xl:px-12 pb-6 rounded-xl my-5 2xl:w-[900px] xl:w-[700px] lg:w-[500px] shadow-md dark:shadow-gray-600 shadow-gray-300">
    <div class="flex gap-x-3 pb-4">
        @if ($image->user->id == Auth()->user()->id)
            <div class="bg-gradient-to-r from-red-400 to-purple-500 rounded-full w-[4.5rem] h-[4.5rem] flex justify-center items-center">
                <div class="rounded-full h-16 w-16 bg-white dark:bg-gray-600 overflow-hidden shadow-md shadow-gray-300 dark:shadow-gray-800">
                    <img src="{{ asset('storage/' . $image->user->settings->avatar) }}" alt="avatar">
                </div>
            </div>
        @else
            <div class="rounded-full h-16 w-16 border-4 border-red-400 overflow-hidden shadow-md shadow-gray-300 dark:shadow-gray-800">
                <img src="{{ asset( 'storage/' .$image->user->settings->avatar) }}" alt="avatar">
            </div>
        @endif
        <div class="flex flex-col justify-center">
            <h3 class="text-base font-semibold text-gray-600 dark:text-gray-100">{{ $image->user->settings->pseudo }}</h3>
        ...
        <div class="text-gray-400 dark:text-gray-300 mr-0 ml-auto text-xs">
